...
# Create a logger
csv_logger = CSVLogger('../weights/transfer_refine_resnet.csv', append=False)

# Create a checkpoint object
model_checkpoint = ModelCheckpoint(
        '../weights/transfer_refine_resnet.h5',
        monitor='val_acc',
        save_best_only=True,
        period=1)
...
# After the desired # of epochs set more of the top layers to 'trainable'
for layer in model.layers[:163]:
    layer.trainable = False
for layer in model.layers[163:]:
    layer.trainable = True

# Recompile so the changes take effect, this time using stochastic gradient descent
model.compile(optimizer=SGD(lr=0.0001, momentum=0.9), 
        loss='binary_crossentropy',
        metrics=['accuracy'])

# Set the logger to append to the file rather than overwrite
csv_logger = CSVLogger('weights/transfer_refine_resnet.csv', append=True)

# Fit again for the same number of epochs
model.fit_generator(
    train_generator,
    steps_per_epoch=nb_train_samples // batch_size,
    epochs=epochs,
